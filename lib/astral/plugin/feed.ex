defmodule Astral.Plugin.Feed do
  ...
  defp render(site, opts) do
    ...
    author = Keyword.get(opts, :author, "Astral")
  ...
  defp entries(site, opts) do
    ...
    site.entries
    |> Map.get(collection, [])
    |> Enum.sort_by(&entry_datetime/1, {:desc, DateTime})
    |> Enum.take(limit)
  end
  ...
  defp entry_datetime(entry), do: metadata_datetime(entry, "updated")
  ...
  defp entry_summary(entry),
    do: Map.get(entry.data, :description, entry.content.title || entry.data.title)
